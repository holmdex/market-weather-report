...
monthly_weather_summary <- monthly_data %>%
  ...
  mutate(
    weather_icon = case_when(
      last_return >= 5 ~ "🌞",        # Exceptional monthly gain
      last_return >= 3 ~ "🌤️",        # Strong monthly gain
      last_return >= 1 ~ "⛅",         # Moderate monthly gain
      last_return >= 0 ~ "🌥️",        # Slight monthly gain
      last_return >= -1 ~ "☁️",       # Stable/minor decline
      last_return >= -3 ~ "🌧️",       # Moderate decline
      last_return >= -5 ~ "⛈️",       # Significant decline
      last_return >= -7 ~ "🌪️",       # Major decline
      TRUE ~ "⚡"                      # Severe decline
    ),
...
yearly_weather_summary <- yearly_data %>%
  ...
  mutate(
    weather_icon = case_when(
      total_return >= 20 ~ "🌞",      # Exceptional year
      total_return >= 15 ~ "🌤️",      # Very strong year
      total_return >= 10 ~ "⛅",       # Strong year
      total_return >= 5 ~ "🌥️",       # Positive year
      total_return >= 0 ~ "☁️",       # Stable year
      total_return >= -5 ~ "🌧️",      # Challenging year
      total_return >= -10 ~ "⛈️",     # Difficult year
      total_return >= -15 ~ "🌪️",     # Tough year
      TRUE ~ "⚡"                      # Critical year
    ),
    conditions = case_when(
...
- 🌞 Bright and Sunny: Exceptional performance
- 🌤️ Mostly Sunny: Very strong performance
- ⛅ Partly Sunny: Strong performance
- 🌥️ Mostly Cloudy: Positive but moderate
- ☁️ Cloudy: Stable/neutral performance
- 🌧️ Light Rain: Minor weakness
- ⛈️ Storm: Significant weakness
- 🌪️ Tornado: Major turbulence
- ⚡ Lightning: Severe market stress
...
monthly_plot <- plot_ly(monthly_data, x = ~date, y = ~norm_price, 
  ...
  add_lines(
    ...
    hoverinfo = "text",
    line = list(width = 1.5)  # Thinner lines for better clarity
  ) %>%
  layout(
    title = list(
      text = paste0(
        ...
        "<br>",
        "<sup style='font-size: 0.8em'>Double-Click on market names to focus on specific markets 🔍</sup>"
      ),
    ...
    plot_bgcolor = "#FAFAFA",  # Lighter background
    paper_bgcolor = "#FAFAFA",
    yaxis = list(
      title = list(
        text = "Value (Starting Point = 100)",
        font = list(size = 12, color = "#2f2f2f")
      ),
      tickfont = list(size = 10, color = "#2f2f2f"),
      gridcolor = "#E8E8E8",  # Lighter grid
      fixedrange = FALSE,
      automargin = TRUE,
      tickformat = ".1f",
      zerolinecolor = "#CCCCCC"  # Subtle zero line
    ),
    xaxis = list(
      title = list(
        text = "Date",
    ...
    margin = list(l = 60, r = 60, t = 90, b = 140),  # Adjusted margins
    height = 800,
    showlegend = TRUE,
    legend = list(
      orientation = "h",
      ...
      bordercolor = "#E0E0E0",
      borderwidth = 1,
      font = list(size = 10, color = "#2f2f2f", family = "Arial"),
...
yearly_plot <- plot_ly(yearly_data, x = ~date, y = ~norm_price, 
  ...
  layout(
    ...
    margin = list(l = 60, r = 60, t = 90, b = 140),
    height = 800,
    showlegend = TRUE,
    legend = list(
      orientation = "h",
...
forecasts <- monthly_data %>%
  group_by(index_name) %>%
  group_modify(~{
    ts_data <- ts(.x$norm_price, frequency = 7)
...
forecast_plot <- plot_ly() %>%
  ...
  add_ribbons(data = forecasts,
             ...
             color = ~index_name,
             colors = getOption("plotly.palette")(),
             opacity = 0.12,  # More subtle ribbons
             legendgroup = ~index_name,
             showlegend = FALSE,
             name = "Possible Range",
             text = ~paste(
               index_name,
               "\nDate:", format(date, "%b %d"),
               "\nRange:", round(lower_95, 1), "% to", round(upper_95, 1), "%"
             ),
             hoverinfo = "text") %>%
  add_lines(data = forecasts,
           ...
           color = ~index_name,
           colors = getOption("plotly.palette")(),
           line = list(dash = 'dash', width = 1.25),  # Slightly thinner forecast lines
           legendgroup = ~index_name,
           showlegend = FALSE,
           text = ~paste(
             index_name,
  ...
  layout(
    ...
    shapes = list(
      list(
        ...
        y1 = 1,
        yref = "paper",
        line = list(color = "#957DAD", dash = "dot", width = 1.5)
      )
    ),
    annotations = list(
      list(
        ...
        bgcolor = "white",
        bordercolor = "#957DAD",
        font = list(size = 11, color = "#2f2f2f", family = "Arial")
      )
    ),
    modebar = list(
      bgcolor = "transparent",
...
## The Market's Weekly Rhythm Guide 📊
...
monthly_day_summary <- monthly_data %>%
  ...
  mutate(
    ...
    day_rating = case_when(
      success_rate >= 70 ~ "🌟 Exceptional Day",
      success_rate >= 60 ~ "⭐ Very Strong Day",
      success_rate >= 55 ~ "✨ Strong Day",
      success_rate >= 50 ~ "💫 Positive Day",
      success_rate >= 45 ~ "🌙 Neutral Day",
      success_rate >= 40 ~ "⚡ Challenging Day",
      success_rate >= 35 ~ "🌩️ Difficult Day",
      TRUE ~ "⛈️ Very Challenging Day"
    ),
...
#### Understanding Your Weekly Rhythm Guide 📊

The Weekly Rhythm Guide analyzes how markets typically perform on each trading day. Here's what the indicators mean:

**Success Rate %**:
- 70%+ (🌟): Exceptionally strong historical performance
- 60-70% (⭐): Very reliable positive performance
- 55-60% (✨): Consistently good performance
- 50-55% (💫): Slightly positive tendency
- 45-50% (🌙): Neutral performance
- 40-45% (⚡): Challenging conditions
- 35-40% (🌩️): Historically difficult
- Below 35% (⛈️): Significant historical challenges
...
monthly_summary <- monthly_data %>%
  ...
  mutate(
    market_mood = case_when(
      avg_return > 5 ~ "📈 Exceptional Growth",
      avg_return > 3 ~ "↗️ Strong Momentum",
      avg_return > 1 ~ "📊 Steady Progress",
      avg_return > 0 ~ "➡️ Modest Gains",
      avg_return > -1 ~ "↔️ Holding Pattern",
      avg_return > -3 ~ "↘️ Minor Weakness",
      avg_return > -5 ~ "📉 Significant Pressure",
      TRUE ~ "⚠️ Major Challenges"
    ),
    stability = case_when(
      volatility < 0.5 ~ "🏆 Exceptionally Calm",
      volatility < 1.0 ~ "🌟 Very Stable",
      volatility < 1.5 ~ "⭐ Stable",
      volatility < 2.0 ~ "💫 Moderate Movement",
      volatility < 2.5 ~ "✨ Active Markets",
      volatility < 3.0 ~ "🌊 High Activity",
      TRUE ~ "🌪️ Turbulent Markets"
    ),
...
yearly_summary <- yearly_data %>%
  ...
  mutate(
    market_mood = case_when(
      avg_ytd_return > 20 ~ "📈 Bull Market Leader",
      avg_ytd_return > 15 ~ "↗️ Strong Bull Run",
      avg_ytd_return > 10 ~ "📊 Solid Uptrend",
      avg_ytd_return > 5 ~ "➡️ Positive Trend",
      avg_ytd_return > 0 ~ "↔️ Stable Markets",
      avg_ytd_return > -5 ~ "↘️ Minor Correction",
      avg_ytd_return > -10 ~ "📉 Correction Phase",
      avg_ytd_return > -15 ~ "⚠️ Deep Correction",
      TRUE ~ "🔄 Bear Market Territory"
    ),
    stability = case_when(
      yearly_volatility < 0.5 ~ "🏆 Rock Solid Year",
      yearly_volatility < 1.0 ~ "🌟 Very Stable Year",
      yearly_volatility < 1.5 ~ "⭐ Stable Year",
      yearly_volatility < 2.0 ~ "💫 Moderate Year",
      yearly_volatility < 2.5 ~ "✨ Active Year",
      yearly_volatility < 3.0 ~ "🌊 Volatile Year",
      TRUE ~ "🌪️ Highly Volatile Year"
    ),
    mood_colors = case_when(
...
yearly_table <- data.frame(
  ...
  Value = c(
    ...
    paste0(ifelse(yearly_summary$avg_ytd_return > 0, "🎯 ", "🎲 "),
           case_when(
...
**Market Mood Scale**:
Monthly:
- 📈 Exceptional Growth (>5%)
- ↗️ Strong Momentum (3-5%)
- 📊 Steady Progress (1-3%)
- ➡️ Modest Gains (0-1%)
- ↔️ Holding Pattern (-1-0%)
- ↘️ Minor Weakness (-3 to -1%)
- 📉 Significant Pressure (-5 to -3%)
- ⚠️ Major Challenges (<-5%)

Yearly:
- 📈 Bull Market Leader (>20%)
- ↗️ Strong Bull Run (15-20%)
- 📊 Solid Uptrend (10-15%)
- ➡️ Positive Trend (5-10%)
- ↔️ Stable Markets (0-5%)
- ↘️ Minor Correction (-5-0%)
- 📉 Correction Phase (-10 to -5%)
- ⚠️ Deep Correction (-15 to -10%)
- 🔄 Bear Market (<-15%)

**Volatility Levels**:
- 🏆 Exceptionally Calm: Minimal fluctuations
- 🌟 Very Stable: Small, orderly movements
- ⭐ Stable: Normal market activity
- 💫 Moderate: Typical volatility
- ✨ Active: Increased movement
- 🌊 High: Notable volatility
- 🌪️ Turbulent: Significant swings
...
/* Plot styling - maintaining consistent width and spacing */
.plotly {
  margin: 60px 0 80px 0 !important;  /* Increased vertical spacing */
  border-radius: 12px;
  ...
  background: white;
  padding: 15px;
  width: 100% !important;  /* Ensuring full width */
  max-width: 100% !important;  /* Maintaining max width constraint */
}
...
/* Container width control and consistency */
div.main-container {  
  max-width: 95vw;
}
